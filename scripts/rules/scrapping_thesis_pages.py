...
def findFrenchResume(s: BeautifulSoup) -> str:
    ...
    resume_french = s.find_all("span", {"property": "dc:description", "xml:lang": "fr"})
...
def findEnglishResume(s: BeautifulSoup) -> str:
    ...
    resume_eng = s.find_all("span", {"property": "dc:description", "xml:lang": "en"})
...
def findFrenchTitle(s: BeautifulSoup) -> str:
    ...
    title_french = s.find_all("h1", {"property": "dc:title", "xml:lang": "fr"})
...
def findEnglishTitle(s: BeautifulSoup) -> str:
    """Find the french title of the thesis"""
    title_eng = s.find_all(
        "span", {"property": "dcterms:alternative", "xml:lang": "en"}
    )
...
def findKeywordsEnglish(s: BeautifulSoup) -> str:
    keyWords = s.find_all("span", {"property": "dc:subject", "xml:lang": "en"})
...
def findKeywordsFrench(s: BeautifulSoup) -> str:
    keyWords = s.find_all("span", {"property": "dc:subject", "xml:lang": "fr"})
...
if __name__ == "__main__":
    ...
    extra_results = all_basic_info.swifter.set_npartitions(int(sys.argv[3])).apply(
        extract_thesis_info_from_df_row, result_type="expand", axis=1
    )
